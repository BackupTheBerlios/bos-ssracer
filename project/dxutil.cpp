...
// Comments:  Now supports multi resolution. 
//            default advance step is 1/10th second
//            - JJ
//-----------------------------------------------------------------------------
FLOAT __stdcall DXUtil_Timer( TIMER_COMMAND command , int xTimeStep )
{
    ...
    if( m_bUsingQPF )
    {
        ...
        // Advance the timer by 1/xTimeStep seconds
        if( command == TIMER_ADVANCE )
        {
            m_llStopTime += m_llQPFTicksPerSec/xTimeStep;
            return 0.0f;
        }
